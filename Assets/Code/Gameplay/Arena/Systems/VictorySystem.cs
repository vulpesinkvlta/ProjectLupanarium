...
namespace Code.Gameplay
{
    public sealed class VictorySystem
    {
        ...
        /// <summary>
        /// Бой завершён. Точка входа для экрана наград и перехода
        /// к следующей волне.
        /// </summary>
        ...
        public void Tick()
        {
            ...
            using (TickMarker.Auto())
            {
                ...
                if (AlivePlayerUnits > 0)
                {
                    Complete(BattleResult.PlayerVictory);
                    return;
                }

                if (AliveEnemyUnits > 0)
                {
                    Complete(BattleResult.EnemyVictory);
                    return;
                }

                Complete(BattleResult.Draw);
            }
        }

        private void Complete(BattleResult result)
        {
            Result = result;
